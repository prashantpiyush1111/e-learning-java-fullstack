...
public class AssignmentServiceImpl implements AssignmentService {
 private final AssignmentRepository assignmentRepository; private final AssignmentSubmissionRepository submissionRepository; private final SectionRepository sectionRepository; private final UserRepository userRepository;
 ...
 @Override public AssignmentSubmissionResponse submit(Long assignmentId,SubmitAssignmentRequest r,String email){User u=user(email);if(u.getRole().getName()!=Role.RoleName.STUDENT)throw new BadRequestException("Only students can submit assignments");Assignment a=assignment(assignmentId);if(!Boolean.TRUE.equals(a.getPublished()))throw new BadRequestException("Assignment is not published");if(r.getSubmissionUrl()==null&&r.getSubmissionText()==null)throw new BadRequestException("Provide submission URL or text");if(a.getDueAt()!=null&&LocalDateTime.now().isAfter(a.getDueAt()))throw new BadRequestException("Assignment submission deadline has passed");AssignmentSubmission s=submissionRepository.findByAssignmentIdAndStudentId(assignmentId,u.getId()).orElse(AssignmentSubmission.builder().assignment(a).student(u).build());s.setSubmissionUrl(r.getSubmissionUrl());s.setSubmissionText(r.getSubmissionText());s.setSubmittedAt(LocalDateTime.now());s.setStatus(AssignmentSubmission.SubmissionStatus.SUBMITTED);s.setMarks(null);s.setFeedback(null);return AssignmentSubmissionResponse.fromEntity(submissionRepository.save(s));}
